...
case class MapDiffer[K, V](differ: Differ[V] { type DiffResult <: IterableOnce[Difference[V]] }) extends Differ[Map[K, V]] {
  ...
  override def diff(oldValue: Map[K, V], newValue: Map[K, V]): DiffResult = {
    ...
    val changed = (newValue.keySet & oldValue.keySet).toSeq
      .flatMap(key => differ.diff(oldValue = oldValue(key), newValue = newValue(key)).iterator.map(d => key -> d))
